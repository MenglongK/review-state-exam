...
import {useGetProductsQuery} from "@/lib/feature/productEndpoint";
...
export default function ProductCard({product}: ProductCardProps) {
    ...
    return (
        <div className="w-full max-w-sm bg-neutral-primary-soft p-6 border border-default rounded-lg shadow-xs m-4">
            <Link href="#">
                {product.images?.[0] && (
                    <Image className="rounded-base mb-6"
                           src={product.images[0]}
                           alt={product.title} width={500} height={100}/>
                )}
            </Link>
            <div>
                <div className="flex items-center space-x-3 mb-6">
                    <div className="flex items-center space-x-1 rtl:space-x-reverse">
                        <svg className="w-5 h-5 text-fg-yellow" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
